 ...
 */
/* Getter/setter accessor: with an argument it sets xlFormatSet<func_name>,
 * and always returns the current value via xlFormat<func_name>. */
...
/* Write-only accessor for options libxl exposes no getter for (e.g. the
 * whole-cell Border/BorderColor shortcuts). Returns true on success.
 * Kept separate from the getter/setter macro so it never references a
 * non-existent xlFormat<func_name>() reader. */
